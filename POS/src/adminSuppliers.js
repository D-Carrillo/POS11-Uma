...
const AdminSuppliers = () => {
  ...
  const handleDeleteSupplier = async (supplierId) => {
    if (window.confirm('Are you sure you want to delete this supplier?')) {
      try {
        const response = await axios.put(`http://localhost:5000/api/admin/suppliers/${supplierId}/delete`);
        if (response.data.success) {
  ...
  const handleRestoreSupplier = async (supplierId) => {
    if (window.confirm('Are you sure you want to restore this supplier?')) {
      try {
        const response = await axios.put(`http://localhost:5000/api/admin/suppliers/${supplierId}/restore`);
        if (response.data.success) {
